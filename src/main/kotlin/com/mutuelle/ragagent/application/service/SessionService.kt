...
import com.mutuelle.ragagent.domain.model.SessionStatus
...
class SessionService(
     ...
     */
    suspend fun createSession(adherentId: UUID): ConversationSession {
        ...
        saveSession(session)
        logger.info { "Created session: ${session.id} for adherent: $adherentId" }
    ...
    /**
     * Updates session activity timestamp and intent.
     */
    suspend fun updateActivity(sessionId: String, intent: Intent?) {
        val session = getSession(sessionId) ?: return

        val updated = session.withUpdatedActivity(intent)
        saveSession(updated)
    ...
    /**
     * Marks a session as escalated.
     */
    suspend fun escalateSession(sessionId: String, reason: String) {
        val session = getSession(sessionId) ?: return

        val escalated = session.withEscalation(reason)
        saveSession(escalated)

        logger.info { "Escalated session: $sessionId, reason: $reason" }
    }
    ...
    /**
     * Gets all active sessions for an adherent.
     */
    suspend fun getActiveSessionsForAdherent(adherentId: UUID): List<ConversationSession> {
        // Note: In production, you'd want to use Redis SCAN or maintain an index
